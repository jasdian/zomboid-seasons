...
async fn build_payloads(
    ...
    match event {
        ...
        NotificationEvent::IncidentResolved { incident_id } => {
            ...
            let discord =
                json!({ "embeds": [format_incident_resolved(&incident, &components, duration)] });
            let webhook = json!({
...
/// Simple jitter using nanosecond timestamp bits (no rand dependency).
fn apply_jitter(delay: std::time::Duration, percent: u8) -> std::time::Duration {
    if percent == 0 {
        return delay;
    }
    ...
    let jitter_range = ms * (percent as f64 / 100.0);
    let jitter = (nanos as f64 / u32::MAX as f64) * 2.0 * jitter_range - jitter_range;
    std::time::Duration::from_millis((ms + jitter).max(100.0) as u64)
}

/// Parse Discord's Retry-After header (seconds as float).
fn parse_retry_after(resp: &reqwest::Response) -> Option<std::time::Duration> {
    resp.headers()
        .get("retry-after")
        .and_then(|v| v.to_str().ok())
        .and_then(|s| s.parse::<f64>().ok())
        .map(|secs| std::time::Duration::from_secs_f64(secs.clamp(0.5, 120.0)))
}
...
async fn send_with_retry(
    ...
    let max_rate_limit_retries = 3u32;
    ...
    loop {
        ...
        match result {
            Ok(resp) => {
                ...
                // 429: separate budget, respect Retry-After
                if status == 429 {
                    let retry_after =
                        parse_retry_after(&resp).unwrap_or(std::time::Duration::from_secs(5));
                    rate_limit_retries += 1;
                    if rate_limit_retries > max_rate_limit_retries {
                        return Err(format!(
                            "429 rate limited from {url} after {rate_limit_retries} retries"
                        ));
                    }
                    tracing::warn!(
                        url,
                        ...
                        "Discord 429 — waiting Retry-After"
                    );
                    tokio::time::sleep(retry_after).await;
                    attempt -= 1; // don't count 429 against normal budget
                    continue;
                }
                ...
                if status >= 500 && attempt < max_attempts {
                    ...
                    tokio::time::sleep(apply_jitter(delay, 20)).await;
                    delay *= 2;
                    continue;
                }
            ...
            Err(e) => {
                ...
                return Err(format!(
                    "request to {url} failed after {max_attempts} attempts: {e}"
                ));
            }
        }
    }
