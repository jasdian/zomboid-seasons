...
use crate::api::models::{
    ...
    IncidentUpdateResponse, MaintenanceListResponse, MaintenanceResponse, MonthHistoryResponse,
    MonthUptimeResponse, StatusPageResponse, UptimeCalendarQuery, UptimeResponse,
};
...
async fn get_maintenance(
    ...
    let completed_responses: Vec<MaintenanceResponse> =
        completed.into_iter().map(maintenance_to_response).collect();
...
fn build_atom_feed(incidents: &[IncidentResponse], maintenances: &[MaintenanceResponse]) -> String {
    ...
    format!(
        "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n<feed xmlns=\"http://www.w3.org/2005/Atom\">\n  <title>Zomboid Seasons Status</title>\n  <id>https://status.princeofcrypto.com/</id>\n  <link href=\"https://status.princeofcrypto.com/\" rel=\"alternate\"/>\n  <link href=\"https://status.princeofcrypto.com/api/status/feed.atom\" rel=\"self\"/>\n  <updated>{now}</updated>\n{entries}</feed>\n"
    )
}
