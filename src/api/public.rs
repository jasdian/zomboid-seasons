...
/// Wraps a response with a Cache-Control max-age header.
struct Cached<T>(T, u32);
...
impl<T: IntoResponse> IntoResponse for Cached<T> {
    fn into_response(self) -> axum::response::Response {
        ...
        resp.headers_mut().insert(
            ...
            format!("public, max-age={}", self.1).parse().unwrap(),
        );
        resp
    }
}
...
async fn get_current_season(State(state): State<AppState>) -> AppResult<impl IntoResponse> {
...
async fn list_seasons(State(state): State<AppState>) -> AppResult<impl IntoResponse> {
    let seasons = state.repo.list_seasons().await?;
    let summaries: Vec<SeasonSummary> = seasons
        .iter()
        .map(|s| SeasonSummary {
            id: s.id,
            status: s.status,
...
async fn get_supply_drops(State(state): State<AppState>) -> AppResult<impl IntoResponse> {
    let season = state.repo.get_active_season().await?;
    ...
    Ok(Cached(Json(responses), 15))
}

async fn get_gametime(State(state): State<AppState>) -> AppResult<impl IntoResponse> {
    let guard = state.gametime_cache.read().await;
    let json = match &*guard {
        Some(gt) => serde_json::json!({
            "month": gt.month,
            "day": gt.day,
            "hour": gt.hour,
            "minute": gt.minute,
