...
/// Whitelist a player on the PZ Build 42 server via RCON.
///
/// Build 42 dual-layer whitelisting:
/// 1. `addsteamid <steam_id>` — gates whether the player can connect at all (Open=false)
/// 2. `adduser <username> <password>` — creates the in-game account
pub async fn whitelist_player(
    ...
    let cmd = format!("addsteamid {}", steam_id);
...
pub async fn remove_player(
    ...
    let cmd = format!("removesteamid {}", steam_id);
    rcon_command_best_effort(rcon_config, &cmd).await;
}
