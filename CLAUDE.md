...
### Known Issue: FIFO Stale Command Loop

The PZ server reads stdin from a FIFO pipe (`zomboid.control`) managed by `zomboid.socket`. The `ExecStop` writes `save` then `quit` to this FIFO. If the server has already exited when `ExecStop` runs (crash, manual kill), those commands remain buffered in the pipe. On next start, the server reads the stale `save`/`quit` and immediately shuts down, creating a restart loop.

**Fix:** Stop both units, delete the FIFO, restart: `systemctl stop zomboid.service && systemctl stop zomboid.socket && rm -f /home/pzuser/pzserver/zomboid.control && systemctl start zomboid.socket && systemctl start zomboid.service`
