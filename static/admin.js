...
async function auth(){
  ...
  const fb=$('#auth-fb');
  try{
    const r=await fetchRetry('/api/admin/promo',{headers:{'Authorization':`Bearer ${t}`}});
...
async function loadHeaderSeason(){
  try{
    const r=await fetchRetry('/api/season');if(!r.ok)return;
...
async function loadPromos(){
  try{
    const r=await fetchRetry('/api/admin/promo',{headers:hd()});if(!r.ok)throw 0;
...
async function createPromo(e){
  ...
  try{
    const r=await fetchRetry('/api/admin/promo',{method:'POST',headers:hd(),body:JSON.stringify(body)});
...
async function revokePromo(code){
  ...
  try{const r=await fetchRetry(`/api/admin/promo/${encodeURIComponent(code)}`,{method:'DELETE',headers:hd()});
    if(r.ok||r.status===204)loadPromos()}catch{}
}

async function loadSeasons(){
  try{const r=await fetchRetry('/api/seasons');if(!r.ok)return;const ss=await r.json();
...
async function loadRegs(){
  try{
    ...
    const r=await fetchRetry(url,{headers:hd()});
...
async function revokeRegistration(id,name){
  ...
  try{
    const r=await fetchRetry(`/api/admin/registrations/${encodeURIComponent(id)}`,{method:'DELETE',headers:hd()});
...
async function forceRotate(){
  ...
  try{
    const r=await fetchRetry('/api/admin/rotate',{method:'POST',headers:hd()});const d=await r.json();
...
async function loadDrops(){
  try{
    const r=await fetchRetry('/api/admin/supply-drops',{headers:hd()});if(!r.ok)throw 0;
...
async function loadDropPois(){
  try{
    const r=await fetchRetry('/api/supply-drops/pois');if(!r.ok)return;
...
async function triggerDrop(e){
  ...
  try{
    const r=await fetchRetry('/api/admin/supply-drops',{method:'POST',headers:hd(),body:JSON.stringify(body)});
    const d=await r.json();
    if(!r.ok){fb.className='fb-err';fb.textContent=d.error||'Failed';return}
...
async function reloadLua(){
  ...
  try{
    const r=await fetchRetry('/api/admin/reload-lua',{method:'POST',headers:hd()});const d=await r.json();
    if(!r.ok){fb.className='fb-err';fb.textContent=d.error||'Failed'}
