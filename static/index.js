...
async function fetchRetry(url,opts={},retries=3){
  for(let i=0;i<retries;i++){
    try{
      ...
      if(r.status===429){
        const w=Math.min(1000*Math.pow(2,i),8000);
...
async function loadSeason(){
  try{
    const r=await fetchRetry('/api/season');if(!r.ok)throw 0;
    ...
    $('#s-title').textContent='Season Unavailable';
    setTimeout(loadSeason,5000);
  }
}

async function loadSeasons(){
  try{
    const r=await fetchRetry('/api/seasons');if(!r.ok)throw 0;
...
$('#reg-form').addEventListener('submit',async e=>{
  ...
  const body={zomboid_username:$('#f_name').value.trim(),zomboid_password:$('#f_pass').value};
  const steam=$('#f_steam').value.trim();if(steam)body.steam_id=steam;
  ...
  try{
    const r=await fetchRetry('/api/register',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
    let d;
...
$('#chk-form').addEventListener('submit',async e=>{
  ...
  try{
    const r=await fetchRetry(`/api/register/${encodeURIComponent(id)}`);
...
async function loadSupplyDrops(){
  try{
    const r=await fetchRetry('/api/supply-drops');if(!r.ok)throw 0;
...
async function loadGameTime(){
  try{
    const r=await fetchRetry('/api/gametime');if(!r.ok)return;
...
loadSeason().then(()=>{loadSeasons();loadSupplyDrops();loadGameTime()});
